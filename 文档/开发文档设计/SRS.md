...
[修订记录 3](#_Toc4736_WPSOffice_Level1)

[审核记录 4](#_Toc1472_WPSOffice_Level1)

[一、 范围 5](#_Toc31959_WPSOffice_Level1)

>   [1. 标识 5](#_Toc13309_WPSOffice_Level2)

>   [2. 系统概述 6](#_Toc19579_WPSOffice_Level2)

>   [3. 文档概述 8](#_Toc20388_WPSOffice_Level2)

>   [4. 基线 8](#_Toc3310_WPSOffice_Level2)

[二、 引用文件 8](#_Toc15022_WPSOffice_Level1)

[三、 需求 9](#_Toc4803_WPSOffice_Level1)

>   [1. 所需的状态和方式 9](#_Toc28666_WPSOffice_Level2)

>   [2. 需求概述 9](#_Toc19365_WPSOffice_Level2)

>   [3. 需求规格 13](#_Toc13219_WPSOffice_Level2)

>   [4. CSCI能力需求 18](#_Toc19420_WPSOffice_Level2)

>   [5. 保密性和隐私性需求 71](#_Toc3844_WPSOffice_Level2)

>   [6. CSCI环境需求 72](#_Toc27844_WPSOffice_Level2)

>   [7. 性能需求 72](#_Toc15821_WPSOffice_Level2)

>   [8. 计算机资源需求 73](#_Toc11268_WPSOffice_Level2)

>   [9. 软件质量因素 75](#_Toc7386_WPSOffice_Level2)

>   [10. 设计和实现的约束 76](#_Toc14738_WPSOffice_Level2)

>   [11. 数据 76](#_Toc28846_WPSOffice_Level2)

>   [12. 操作 76](#_Toc21188_WPSOffice_Level2)

>   [13. 故障处理 77](#_Toc18611_WPSOffice_Level2)

>   [14. 算法说明 77](#_Toc15104_WPSOffice_Level2)

>   [15. 有关人员需求 78](#_Toc11709_WPSOffice_Level2)

>   [16. 有关后勤需求 78](#_Toc26310_WPSOffice_Level2)

>   [17. 需求的优先次序和关键程度 78](#_Toc32096_WPSOffice_Level2)

[四、 合格性规定 80](#_Toc21541_WPSOffice_Level1)

[五、 需求可追踪性 83](#_Toc8460_WPSOffice_Level1)

[六、 尚未解决的问题 83](#_Toc9190_WPSOffice_Level1)

[七、 注解 83](#_Toc1817_WPSOffice_Level1)

[附录A：词汇表 83](#_Toc24810_WPSOffice_Level1)

**修订记录**

| **版本号** | **修订时间** | **修订内容**                                                                             | **提出人**             | **审核人** | **是否通过** |
|------------|--------------|------------------------------------------------------------------------------------------|------------------------|------------|--------------|
| **V1.0.0** | **7月3日**   | **增加需求**                                                                             | **宋昌 熊明光 陆韬韬** | **李宁生** | **是**       |
| **V2.0.0** | **7月6日**   | **添加了任务功能流程，添加了消息流程，添加了相应功能**                                   | **李宁生**             | **卢延悦** | **是**       |
| **V2.1.0** | **7月9日**   | **根据第二次迭代会议，完善，修改需求，并完善需求规格说明书**                             | **李宁生**             | **卢延悦** | **是**       |
| **V3.0.0** | **7月11日**  | **根据第三次迭代会议讨论内容，对需求项做了最后的修改和确认，完成了需求规格说明书最终版** | **李宁生，宋昌**       | **卢延悦** | **是**       |
...
| **V1.1.0**     | **7.3**      | **7.3日增加的需求内容和需求变更内容** | **增加需求的第2项，该需求计划在v2.0版本之后实现。 其余部分进行修改。** | **李宁生** | **是**       |
| **V1.1.0**     | **7.4**      | **7.4日部分用户逻辑功能进行修改**     | **需要进行将部分逻辑功能进行修改**                                     | **李宁生** | **是**       |
| **V1.1.0**     | **7.6**      | **查看是否符合最新需求规格说明**      | **系统结构图需要更新，需求规格说明部分需要更新**                       | **李宁生** | **是**       |
| **V2.0.0**     | **7.10**     | **SRS需求规格,CSCI是否符合标准**      | **CSCI等需要根据最新需求规格书写**                                     | **李宁生** | **是**       |
| **v2.0.0**     | **7.10**     | **一、1.2 术语和缩略语**              | **项目全称写错啦+++**                                                  | **李新博** | **是**       |
...
| **v3.0.0**     | **7.11**     | **三、4.1.1.1 三、4.1.1.3**           | **用户在进行和密码有关操作的时候，全部进行md5加密**                    | **宋昌**   | **是**       |
| **v3.0.0**     | **7.11**     | **三、4.3.3.1**                       | **申请人被指派后，任务相关申请全部清除**                               | **宋昌**   | **是**       |
| **v3.0.0**     | **7.11**     | **三、4.3.4**                         | **任务热度的判断由点亮数改为评论数**                                   | **宋昌**   | **是**       |
| **v3.0.0**     | **7.11**     | **三、4.3.2.1**                       | **加入评价模块，改为星星评价模式**                                     | **宋昌**   | **是**       |
| **v3.0.0**     | **7.11**     | **三、4.1.3**                         | **用户私信需求，放弃使用webSocket，改用轮询方式加数据库状态控制。**    | **宋昌**   | **是**       |
...
>   技术：通过对数据库消息条目进行检索并检查有效性，将其显示，通过私信指定人可以跳入指定聊天窗口，实现在线聊天。（此处的技术由webSocket改成页面轮询数据库处理）
...
>   背景：是发布者对任务完成情况的满意程度的体现，此处的任务评价形式修改成星星修改，即五星选择
...
| 申请列表结果 | 申请数据库 | 一次多个 | 一个字符 | 无       | 0/1          | 空值已处理   | 输出错误 |
...
| 13   | 配置-数据库建立                       | 5             | 关键     |
| 14   | 配置-框架搭建                         | 5             | 关键     |
| 15   | 页面设计-user 登录，注册，忘记密码    | 4             | 重要     |
| 16   | 页面设计-admin 登录，注册，忘记密码   | 4             | 重要     |
| 17   | 页面设计-user 主页                    | 4             | 重要     |
| 18   | 页面设计-user 消息列表                | 4             | 重要     |
| 19   | 页面设计-user 消息发送                | 4             | 重要     |
| 20   | 页面设计-user 个人信息页面            | 4             | 重要     |
| 21   | 页面设计-user 7种任务状态显示         | 4             | 重要     |
| 22   | 页面设计-user 发布任务页面            | 4             | 重要     |
| 23   | 页面设计-user 任务详情页面            | 4             | 重要     |
| 24   | 页面设计-user 公告详细页面            | 3             | 一般     |
| 25   | 页面设计-user 评价                    | 3             | 一般     |
| 26   | 页面设计-user 他人个人信息页面        | 4             | 重要     |
| 27   | 页面设计-admin 任务管理页面           | 4             | 重要     |
| 28   | 页面设计-admin 公告管理页面           | 4             | 重要     |
| 29   | 页面设计-admin 系统参数页面           | 4             | 重要     |
| 30   | 页面设计-admin 用户管理页面           | 4             | 重要     |
| 31   | 页面设计-admin 个人修改密码页面       | 4             | 重要     |
| 32   | 页面设计-admin 个人修改其他属性页面   | 4             | 重要     |
| 33   | 功能-user 登录，注册，忘记密码        | 5             | 关键     |
| 34   | 功能-admin 登录，注册，忘记密码       | 5             | 关键     |
| 35   | 功能-user 个人信息管理                | 3             | 一般     |
| 36   | 功能-user 个人任务7种状态展示         | 4             | 重要     |
| 37   | 功能-admin 任务通过，删除             | 3             | 一般     |
| 38   | 功能-admin 公告添加，删除，修改，查看 | 3             | 一般     |
| 39   | 功能-admin 系统参数修改               | 3             | 一般     |
| 40   | 功能-admin 用户增加，封号，积分修改   | 3             | 一般     |
| 41   | 功能-admin admin修改修改密码          | 3             | 一般     |
| 42   | 功能-admin admin修改邮箱，名字        | 3             | 一般     |
| 43   | 功能-user 展示已读，未读消息          | 3             | 一般     |
| 44   | 功能-user 与另外一用户进行私信功能    | 3             | 一般     |
| 45   | 功能-user 主页：展示所有可以接受任务  | 4             | 重要     |
| 46   | 功能-user 发布任务                    | 4             | 重要     |
| 47   | 功能-user 查看未接取任务详情          | 4             | 重要     |
| 48   | 功能-user 查看与自己有关所有任务详情  | 4             | 重要     |
| 49   | 功能-user 对任务进行评论              | 3             | 一般     |
| 50   | 功能-user 对已完成任务进行评价        | 3             | 一般     |
| 51   | 维护-数据库维护                       | 5             | 关键     |
| 52   | 维护-前端页面维护                     | 5             | 关键     |
| 53   | 测试-后端单元测试                     | 5             | 关键     |
| 54   | 安全-数据库用户密码安全               | 4             | 重要     |
| 55   | 安全-SSL                              | 4             | 重要     |
| 56   | 安全-消息传递安全机制                 | 4             | 重要     |
| 57   | 部署-云服务器部署                     | 4             | 重要     |
| 58   | 维护-功能维护                         | 5             | 关键     |
...
| 测试用例编号               | 01                   | 测试需求项                               | H2H在线互助系统                                        | 日期 | 2019/7/11 |
|----------------------------|----------------------|------------------------------------------|--------------------------------------------------------|------|-----------|
...
| 1                          | 登录                 | 检查用户名、密码、验证码                 | 是否允许登入                                           |      |           |
| 2                          | 找回密码             | 检查安全信息、验证码                     | 是否允许更改密码                                       |      |           |
| 3                          | 审核任务             | 检查任务是否符合标准                     | 是否允许通过审核                                       |      |           |
| 4                          | 删除任务             | 删除错误的任务                           | 删除错误的任务                                         |      |           |
| 5                          | 筛选任务             | 点击按钮，按钮有不同的类型               | 筛选出自己想要的任务类型                               |      |           |
| 6                          | 查看任务详情         | 点击任务名                               | 显示出任务的详细情况                                   |      |           |
| 7                          | 用户封号             | 点击封号按钮                             | 对用户进行封号处理，并至用户状态为封号状态             |      |           |
| 8                          | 用户删号             | 点击删号按钮                             | 对用户进行删号处理 ，并至用户状态为删号状态            |      |           |
| 9                          | 用户详情             | 点击用户姓名                             | 显示用户的所有的信息                                   |      |           |
| 10                         | 用户检索             | 点击搜索，对用户搜索进行                 | 显示出用户筛选结果                                     |      |           |
| 11                         | 修改系统参数         | 点击按钮，可以进行修改积分               | 数据库中积分数值改变                                   |      |           |
| 12                         | 公告发布             | 点击发布公告，填写公告                   | 公告列表中会显示公告                                   |      |           |
| 13                         | 公告修改             | 点击公告修改，进行修改公告               | 公告列表中会显示修改过后的公告                         |      |           |
| 14                         | 公告删除             | 点击公告修改，进行修改公告               | 公告列表中显示更改过后的公告                           |      |           |
| 15                         | 用户信息修改         | 用户信息可以进行用户名修改，以及邮箱修改 | 用户信息中信息发生改变                                 |      |           |
| 16                         | 用户密码修改         | 输入俩次相同密码进行修改密码             | 登录输入密码发生了修改                                 |      |           |
